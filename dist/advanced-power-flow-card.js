...
function normalizeConfig(input) {
	...
	return {
		...
		diagnostics: {
			...
			mppt_daily_relative_warning_enabled: typeof diagnosticsRaw.mppt_daily_relative_warning_enabled === "boolean" ? diagnosticsRaw.mppt_daily_relative_warning_enabled : false,
			mppt_daily_relative_warning_ratio: typeof diagnosticsRaw.mppt_daily_relative_warning_ratio === "number" ? diagnosticsRaw.mppt_daily_relative_warning_ratio : .35,
			pv_temperature_high: typeof diagnosticsRaw.pv_temperature_high === "number" ? diagnosticsRaw.pv_temperature_high : 75,
			stale_sensor_minutes: typeof diagnosticsRaw.stale_sensor_minutes === "number" ? diagnosticsRaw.stale_sensor_minutes : 0
		},
		tariffs: raw.tariffs && typeof raw.tariffs === "object" ? raw.tariffs : void 0,
		...
		flow_routing: raw.flow_routing === "orthogonal" ? "orthogonal" : raw.flow_routing === "natural" || raw.flow_routing === "curved" ? "natural" : "natural",
...
function createStubConfig() {
	return {
		...
		diagnostics: {
			...
			mppt_daily_relative_warning_ratio: .35
		},
		power_threshold: 5,
	...
	render() {
      ...
      <div class="editor">
        <section>
          ...
          ${this._numberInput("Mobile Skalierung", this._config.mobile_scale, 1.06, (value) => this._with((config) => {
			config.mobile_scale = Math.min(1.3, Math.max(.9, value));
		}), {
			min: .9,
			max: 1.3,
			step: .01
          ...
          ${this._checkbox("PV-Systemstatus im Hauptdiagramm anzeigen", this._config.pv_show_status, true, (value) => this._with((config) => {
			config.pv_show_status = value;
		}))}
          ${this._checkbox("Relative PV-Leistung (W/kWp) anzeigen", this._config.pv_show_relative_power, true, (value) => this._with((config) => {
			config.pv_show_relative_power = value;
		}))}
          ${this._checkbox("Leistungsbalken bei kompakten MPPTs anzeigen", this._config.pv_compact_mppt_bars, true, (value) => this._with((config) => {
			config.pv_compact_mppt_bars = value;
		}))}
          ${this._checkbox("PV-Sektion nachts auf eine Zeile reduzieren", this._config.night_pv_collapse, true, (value) => this._with((config) => {
			config.night_pv_collapse = value;
		}))}
          ${this._selectInput("Batterie-Darstellung", this._config.battery_layout, [{
			value: "grouped",
			label: "Gruppiert – gemeinsame Batteriezone"
		}, {
			value: "separate",
			label: "Getrennt"
		}], (value) => this._with((config) => {
			config.battery_layout = value;
		}))}
          ...
          ${this._selectInput("Leitungsführung unten", this._config.flow_routing, [{
			value: "natural",
			label: "Natürlich – direkte weiche Verbindung"
		}, {
			value: "orthogonal",
          ...
          ${this._selectInput("Informationsdichte im Live-Diagramm", this._config.live_detail, [{
			value: "standard",
			label: "Standard – alle Unterzeilen"
		}, {
			value: "minimal",
			label: "Minimal – nur wichtige Zusatzinfos"
		}], (value) => this._with((config) => {
			config.live_detail = value;
		}))}
          ${this._checkbox("Aktive Komponenten stärker hervorheben", this._config.focus_active, true, (value) => this._with((config) => {
			config.focus_active = value;
		}))}
          ...
          ${this._selectInput("Tageswerte-Layout", this._config.daily_layout, [
			...
			{
				value: "strip",
				label: "Kennzahlenleiste – maximal reduziert"
			}
		], (value) => this._with((config) => {
			config.daily_layout = value;
		}))}
          ${this._textInput("Tageskarten – Auswahl/Reihenfolge", (this._config.daily_items ?? []).join(", "), "pv, house, grid-import, grid-export, autarky, self-consumption, self-supplied-energy, self-supply-savings", (value) => this._with((config) => {
			const items = (value ?? "").split(",").map((item) => item.trim()).filter(Boolean);
			config.daily_items = items.length ? items : void 0;
		}))}
        ...
        <section>
          ...
          <div class="form-grid full">
            ${this._entityPicker("Aktueller Bezugspreis", this._config.tariffs?.import_price, (value) => this._with((config) => {
			config.tariffs = {
				...config.tariffs,
				import_price: value
			};
		}))}
            ${this._entityPicker("Aktuelle Einspeisevergütung", this._config.tariffs?.export_price, (value) => this._with((config) => {
			config.tariffs = {
				...config.tariffs,
				export_price: value
			};
		}))}
            ${this._entityPicker("Netzkosten heute", this._config.tariffs?.import_cost_today, (value) => this._with((config) => {
			config.tariffs = {
				...config.tariffs,
				import_cost_today: value
			};
		}))}
            ${this._entityPicker("Einspeiseerlös heute", this._config.tariffs?.export_revenue_today, (value) => this._with((config) => {
			config.tariffs = {
				...config.tariffs,
				export_revenue_today: value
			};
		}))}
            ${this._entityPicker("Eigenversorgungs-Ersparnis heute (optional)", this._config.tariffs?.self_supply_savings_today, (value) => this._with((config) => {
			config.tariffs = {
				...config.tariffs,
				self_supply_savings_today: value
			};
		}))}
            ${this._optionalNumberInput("Fester Bezugspreis [€/kWh]", this._config.tariffs?.fixed_import_price, "optional", (value) => this._with((config) => {
			config.tariffs = {
				...config.tariffs,
				fixed_import_price: value
			};
		}), {
			min: 0,
			step: .001
		})}
            ${this._optionalNumberInput("Feste Einspeisevergütung [€/kWh]", this._config.tariffs?.fixed_export_price, "optional", (value) => this._with((config) => {
			config.tariffs = {
				...config.tariffs,
				fixed_export_price: value
			};
		}), {
			min: 0,
			step: .001
		})}
            ${this._textInput("Währung", this._config.tariffs?.currency, "€", (value) => this._with((config) => {
			config.tariffs = {
				...config.tariffs,
				currency: value
			};
        ...
        <section>
          ...
          <div class="form-grid full">
            ...
            ${this._numberInput("PV/Wechselrichter Temperatur Maximum [°C]", this._config.diagnostics?.pv_temperature_high, 75, (value) => this._with((config) => {
			config.diagnostics = {
				...config.diagnostics,
				pv_temperature_high: value
			};
		}))}
            ${this._numberInput("Sensor veraltet nach [min] (0 = aus)", this._config.diagnostics?.stale_sensor_minutes, 0, (value) => this._with((config) => {
			config.diagnostics = {
				...config.diagnostics,
				stale_sensor_minutes: value
			};
		}), {
			min: 0,
			step: 1
            ...
            ${this._checkbox("MPPT-Tagesertragsdiagnose aktivieren", this._config.diagnostics?.mppt_daily_relative_warning_enabled, false, (value) => this._with((config) => {
			config.diagnostics = {
				...config.diagnostics,
				mppt_daily_relative_warning_enabled: value
			};
		}))}
            ${this._numberInput("Tagesertrags-Warnverhältnis", this._config.diagnostics?.mppt_daily_relative_warning_ratio, .35, (value) => this._with((config) => {
			config.diagnostics = {
				...config.diagnostics,
				mppt_daily_relative_warning_ratio: value
			};
		}), { step: .05 })}
...
var AdvancedPowerFlowCard = class extends i {
	...
	_detailsOpen() {
		return this._heatExpanded || this._expandedBattery !== void 0 || this._pvDailyExpanded || this._expandedPvSystem !== void 0 || this._dailyBalanceExpanded || this._diagnosticsExpanded;
	}
	_state(entityId) {
	...
	_pvSystemDailyEnergyWh(system) {
		const direct = this._energyWh(system.daily_energy);
		if (direct !== void 0) return direct;
		const children = system.children ?? [];
		if (!children.length || children.some((child) => !child.daily_energy)) return void 0;
	...
	_pvDailyTotalWh() {
		...
		if (!systems.length) return void 0;
	...
	_mpptSpecificYield(input) {
		const energyWh = this._energyWh(input.daily_energy);
		if (energyWh === void 0 || !input.installed_kwp || input.installed_kwp <= 0) return void 0;
		return energyWh / 1e3 / input.installed_kwp;
	}
	_heatPumpDailyCopInfo(hp) {
		...
		if (thermalWh === void 0 || electricWh === void 0 || electricWh <= 0) return { calculated: true };
		return {
			value: thermalWh / electricWh,
			calculated: true
		};
	}
	_batteryDailyEnergy(field) {
		const batteries = this._config.batteries ?? [];
	...
	_dailyBalanceInfo() {
		...
		const complete = pv !== void 0 && gridImport !== void 0 && gridExport !== void 0 && house !== void 0 && charge.complete && discharge.complete && charge.value !== void 0 && discharge.value !== void 0;
		const residual = complete ? pv + gridImport + discharge.value - gridExport - charge.value - house : void 0;
		return {
			pv,
			gridImport,
			gridExport,
	...
	_entityStale(entity) {
		...
		return Number.isFinite(ageMs) && ageMs > minutes * 6e4;
	}
	_statusLooksBad(entity) {
	...
	_formatMoney(value, estimated = false) {
		if (value === void 0 || !Number.isFinite(value)) return "—";
		return `${estimated ? "≈ " : ""}${value.toLocaleString(void 0, {
			minimumFractionDigits: 2,
			maximumFractionDigits: 2
		})} ${this._currency()}`;
	}
	_formatMoneyEntity(entity) {
		if (!entity) return "—";
		const value = this._number(entity);
		if (value === void 0) return this._formatMeasurement(entity);
	...
	_estimatedDailyMoney(energyEntity, fixedPrice) {
		if (!energyEntity || fixedPrice === void 0 || fixedPrice < 0) return void 0;
		const energyWh = this._energyWh(energyEntity);
		return energyWh === void 0 ? void 0 : energyWh / 1e3 * fixedPrice;
	}
	_pricePerKwh(entity) {
		const value = this._number(entity);
		...
		if (unit.includes("/mwh")) return value / 1e3;
		return value;
	}
	_dailySelfSupplyInfo() {
		const balance = this._dailyBalanceInfo();
		let houseWh = balance.house;
		const gridImportWh = balance.gridImport;
		if (houseWh === void 0 && balance.pv !== void 0 && balance.gridImport !== void 0 && balance.gridExport !== void 0 && balance.batteryCharge !== void 0 && balance.batteryDischarge !== void 0) houseWh = Math.max(0, balance.pv + balance.gridImport + balance.batteryDischarge - balance.gridExport - balance.batteryCharge);
		const energyWh = houseWh !== void 0 && gridImportWh !== void 0 ? Math.max(0, houseWh - gridImportWh) : void 0;
		const explicitEntity = this._config.tariffs?.self_supply_savings_today;
		const explicitSavings = this._number(explicitEntity);
		if (explicitEntity && explicitSavings !== void 0) return {
			energyWh,
			savings: explicitSavings,
			savingsEntity: explicitEntity,
			estimated: false
		};
		if (energyWh === void 0) return { estimated: false };
		const fixedPrice = this._config.tariffs?.fixed_import_price;
		if (fixedPrice !== void 0 && fixedPrice >= 0) return {
			energyWh,
			savings: energyWh / 1e3 * fixedPrice,
			estimated: false,
			pricePerKwh: fixedPrice
		};
		const importCost = this._number(this._config.tariffs?.import_cost_today);
		if (importCost !== void 0 && gridImportWh !== void 0 && gridImportWh > 1) {
			const averagePrice = importCost / (gridImportWh / 1e3);
			if (Number.isFinite(averagePrice) && averagePrice >= 0) return {
				energyWh,
				savings: energyWh / 1e3 * averagePrice,
				estimated: true,
				pricePerKwh: averagePrice
			};
		}
		const currentPrice = this._pricePerKwh(this._config.tariffs?.import_price);
		if (currentPrice !== void 0) return {
			energyWh,
			savings: energyWh / 1e3 * currentPrice,
			estimated: true,
	...
	_batteryHealthLabel(battery) {
		...
		if (soh !== void 0 && soh < 70) return "SOH niedrig";
		if (soh !== void 0 && soh < 85) return "Beobachten";
		return "Unauffällig";
	}
	_pvHealthLabel(system) {
	...
	_mpptDailyDiagnostic(system, input, inputIndex) {
		const diag = this._config.diagnostics;
		if (diag?.enabled === false || !(diag?.mppt_daily_relative_warning_enabled ?? false)) return void 0;
		const children = system.children ?? [];
		if (children.length < 2) return void 0;
		...
		const ratio = Math.max(0, diag?.mppt_daily_relative_warning_ratio ?? .35);
		if (median > .05 && own < median * ratio) return "Tagesertrag pro kWp deutlich unter den anderen MPPTs";
	}
	...
	_pvSystemWarnings(system) {
		...
		if (inverterTemp !== void 0 && inverterTemp > (this._config.diagnostics?.pv_temperature_high ?? 75)) warnings.push(`Wechselrichtertemperatur hoch (${inverterTemp.toLocaleString(void 0, { maximumFractionDigits: 1 })} °C)`);
	...
	_diagnosticMessages() {
		...
		else if (this._config.grid?.power && this._entityStale(this._config.grid.power)) messages.push({
			severity: "warning",
			title: "Netz",
			detail: "Leistungssensor veraltet",
			nodeId: "grid"
		});
		...
		else if (this._config.house?.power && this._entityStale(this._config.house.power)) messages.push({
			severity: "warning",
			title: this._config.house.name ?? "Haus",
			detail: "Leistungssensor veraltet",
			nodeId: "house"
	...
	_dailyItems() {
		...
		if (tariffs?.import_cost_today) items.push({
			...
			if (estimated !== void 0) items.push({
				key: "import-cost",
				label: "Kosten heute",
				value: this._formatMoney(estimated, true)
			});
		}
		if (tariffs?.export_revenue_today) items.push({
			key: "export-revenue",
		...
		return selected.map((key) => byKey.get(key)).filter((item) => Boolean(item));
	}
	...
	_pvStatusText(system) {
		if (this._config.pv_show_status === false || !system.status) return void 0;
		const state = this._state(system.status)?.state;
		if (!state || state === "unknown" || state === "unavailable") return void 0;
		return {
			on: "Aktiv",
			off: "Aus",
			running: "Aktiv",
			...
			fault: "Fehler",
			error: "Fehler"
		}[state.trim().toLowerCase()] ?? this._short(state, 14);
	}
	_pvRelativePowerText(system, powerW) {
		if (this._config.pv_show_relative_power === false) return void 0;
		if (powerW === void 0 || !system.installed_kwp || system.installed_kwp <= 0) return void 0;
		return `${(Math.max(0, powerW) / system.installed_kwp).toLocaleString(void 0, { maximumFractionDigits: 0 })} W/kWp`;
	}
	_compactMpptItems(system) {
		const children = system.children ?? [];
		if (!children.length) return [];
	...
	_layout() {
		...
		const nightCollapsed = Boolean(solar.length && this._config.night_pv_collapse !== false && this._isPvNight());
		const width = 1e3;
		...
		if (nightCollapsed) {
			...
			const maxCompactChildren = showCompactMppts ? Math.min(6, Math.max(0, ...solar.map((system) => system.children?.length ?? 0))) : 0;
			const compactRows = Math.ceil(maxCompactChildren / 2);
			const compactBarsExtra = this._config.pv_compact_mppt_bars === false ? 0 : 5;
			const parentH = (showCompactMppts ? 88 + compactRows * (27 + compactBarsExtra) : 86) * sizeFactor;
			...
			for (let rowStart = 0; rowStart < solar.length; rowStart += pvColumns) {
				...
				rowSystems.forEach((system, localIndex) => {
					...
					const childW = childColumns === 1 ? Math.min(220, clusterWidth - clusterPadX * 2) : (clusterWidth - clusterPadX * 2 - childGapX) / 2;
					...
					const infoParts = [this._pvStatusText(system), this._pvRelativePowerText(system, parentPower)].filter((value) => Boolean(value));
					const parent = {
						id: `pv-system-${systemIndex}`,
						title: system.name ?? `PV ${systemIndex + 1}`,
						main: this._formatW(parentPower, true),
						sub: showCompactMppts ? infoParts.length ? infoParts.join(" · ") : `${configuredChildren} MPPT${configuredChildren === 1 ? "" : "s"}` : [configuredChildren ? `${configuredChildren} MPPT${configuredChildren === 1 ? "" : "s"}` : "PV-System", ...infoParts].join(" · "),
						entity: system.power,
						kind: "pv-parent",
						x: parentX,
						y: parentY,
						...
						pvShare,
						compactMppts: showCompactMppts ? this._compactMpptItems(system) : void 0
					};
		...
		const center = {
			...
			sub: supplyMode === "full" ? balance.complete ? balance.calculatedHouse ? "Haus aus Leistungsbilanz" : `${balance.warning ? "⚠ " : ""}Bilanz ${this._formatSignedW(balance.residual)}` : "Bilanz unvollständig" : void 0,
			kind: "center",
			x: width / 2 - centerW / 2,
			y: centerY,
			w: centerW,
		...
		const boundsFor = (nodes, label, kind) => {
			...
			return {
				...
				y: minY - 30,
				width: Math.min(984, maxX - minX + 32),
				height: maxY - minY + 46,
				label,
				kind
			};
		...
		const batteryGroup = this._config.battery_layout === "separate" ? void 0 : boundsFor(batteryNodes, "Batterien", "battery");
		const consumerGroup = consumerNodes.length > 1 ? boundsFor(consumerNodes, "Verbraucher", "consumer") : void 0;
		const bottomRows = Math.max(leftRows, rightRows);
		const groupBottom = Math.max(batteryGroup ? batteryGroup.y + batteryGroup.height : 0, consumerGroup ? consumerGroup.y + consumerGroup.height : 0);
		return {
			width,
			height: bottomRows ? Math.max(bottomStartY + bottomRows * bottomRowH + Math.max(0, bottomRows - 1) * bottomGapY + 30, groupBottom + 18) : centerY + mainRowH + 38,
			center,
			grid,
			house,
			pvClusters,
	...
	_heatPumpSummary(hp) {
		...
		if (temperature !== void 0) {
			...
			parts.push(`${label} ${temperature.toLocaleString(void 0, { maximumFractionDigits: 1 })} ${unit}`);
		}
      ...
      >
        ...
        ${node.compactMppts?.length ? w`
			...
			return w`
                  ...
                  ${this._config.pv_compact_mppt_bars !== false ? w`
	...
	_bottomBusPath(from, to, source, width) {
		const x1 = from.x + from.w / 2;
		const y1 = from.y + from.h;
		const x2 = to.x + to.w / 2;
		const y2 = to.y;
		if ((this._config.flow_routing ?? "natural") === "orthogonal") {
			const midY = y1 + Math.max(22, (y2 - y1) * .42);
			return `M ${x1} ${y1} L ${x1} ${midY} L ${x2} ${midY} L ${x2} ${y2}`;
		}
		const distance = Math.max(24, y2 - y1);
		const control = Math.min(70, Math.max(28, distance * .42));
	...
	_pvDailyDetails() {
		...
		const hasSystemData = systems.some((system) => this._pvSystemDailyEnergyWh(system) !== void 0);
		return b`
      <div class="heat-details pv-daily-details">
        ...
        ${hasSystemData ? b`
			...
			const value = valueWh !== void 0 ? entity ? this._formatEnergy(entity) : this._formatEnergyWh(valueWh) : "Nicht konfiguriert";
			...
			return b`
                  <div
                    class=${`detail-item pv-daily-system ${valueWh !== void 0 ? "" : "static missing"}`}
                    @click=${() => {
				if (valueWh === void 0) return;
	...
	_dailyEnergyBalanceDetails() {
		const info = this._dailyBalanceInfo();
		const supply = info.complete ? (info.pv ?? 0) + (info.gridImport ?? 0) + (info.batteryDischarge ?? 0) : void 0;
		const usage = info.complete ? (info.house ?? 0) + (info.gridExport ?? 0) + (info.batteryCharge ?? 0) : void 0;
		const energyCard = (label, value, role) => b`
      ...
      <div class="heat-details daily-balance-details">
        ...
        <div class="daily-balance-columns">
          <div>
            ...
            ${supply !== void 0 ? b`<div class="daily-balance-total"><span>Summe Quellen</span><strong>${this._formatEnergyWh(supply)}</strong></div>` : A}
          </div>
          ...
          <div>
            ...
            ${usage !== void 0 ? b`<div class="daily-balance-total"><span>Summe Verwendung</span><strong>${this._formatEnergyWh(usage)}</strong></div>` : A}
          </div>
        </div>

        ${(() => {
			const selfSupply = this._dailySelfSupplyInfo();
			if (selfSupply.energyWh === void 0) return A;
			const savingsText = selfSupply.savingsEntity ? this._formatMoneyEntity(selfSupply.savingsEntity) : selfSupply.savings !== void 0 ? this._formatMoney(selfSupply.savings, selfSupply.estimated) : "—";
			return b`
            <div class="self-supply-card">
              ...
              ${selfSupply.pricePerKwh !== void 0 ? b`<small>Bewertet mit ${selfSupply.pricePerKwh.toLocaleString(void 0, {
				minimumFractionDigits: 3,
				maximumFractionDigits: 3
			})} ${this._currency()}/kWh${selfSupply.estimated ? " (Näherung)" : ""}</small>` : b`<small>Für eine Geldersparnis festen Bezugspreis oder eine Ersparnis-Entity konfigurieren.</small>`}
            </div>
          `;
		})()}

        ${info.complete && info.residual !== void 0 ? b`
        ...
        ${this._config.daily?.house_energy ? b`
	...
	_pvSystemDetails(system, index) {
      ...
      <div class="heat-details pv-system-details">
        ...
        <div class="detail-grid system-overview-grid">
          ...
          ${system.daily_energy ? this._detailItem("Produktion heute", system.daily_energy) : (() => {
			const systemDailyWh = this._pvSystemDailyEnergyWh(system);
			return systemDailyWh !== void 0 ? this._detailValueItem("Produktion heute", this._formatEnergyWh(systemDailyWh), "Summe der MPPT-Tageswerte") : A;
		})()}
        ...
        ${(system.children ?? []).length ? b`
			...
			const systemDailyWh = this._pvSystemDailyEnergyWh(system);
			const dailyShare = dailyWh !== void 0 && systemDailyWh !== void 0 && systemDailyWh > 0 ? this._clampPercent(dailyWh / systemDailyWh * 100) : void 0;
			const dailySpecificYield = this._mpptSpecificYield(input);
			const warnings = [this._mpptDiagnostic(system, input, childIndex), this._mpptDailyDiagnostic(system, input, childIndex)].filter((value) => Boolean(value));
			return b`
                  <div class=${`mppt-detail-row ${warnings.length ? "warning" : ""}`}>
                    <div class="mppt-detail-head">
                      <div>
                        <strong>${input.name ?? `MPPT ${childIndex + 1}`}</strong>
                        ${warnings.length ? b`<small>⚠ ${warnings.join(" · ")}</small>` : b`<small>Keine Auffälligkeit</small>`}
                      </div>
                    ...
                    <div class="mppt-detail-values">
                      ...
                      ${input.daily_energy ? b`<span><b>${this._formatEnergy(input.daily_energy)}</b> heute</span>` : A}
                      ${dailySpecificYield !== void 0 ? b`<span><b>${dailySpecificYield.toLocaleString(void 0, { maximumFractionDigits: 2 })} kWh/kWp</b> Tagesertrag</span>` : A}
                      ${dailyShare !== void 0 ? b`<span><b>${this._formatPercent(dailyShare)}</b> Tagesanteil</span>` : A}
                      ${input.daily_peak_power ? b`<span><b>${this._formatPower(input.daily_peak_power, true)}</b> Peak heute</span>` : A}
                      ${input.status ? b`<span><b>${this._formatMeasurement(input.status)}</b> Status</span>` : A}
                    </div>
                    ${mpptShare !== void 0 ? b`<div class="pv-share-track"><i style=${`width:${mpptShare}%`}></i></div>` : A}
                    ${dailyShare !== void 0 ? b`<div class="mppt-daily-share">
	...
	_heatDetails(hp) {
      ...
      <div class="heat-details">
        ...
        ${hasAny ? b`
            <div class="detail-grid">
              ${hp.display_temperature && hp.display_temperature !== hp.flow_temperature ? this._detailItem(hp.display_temperature_label?.trim() || "Diagrammtemperatur", hp.display_temperature, "°C") : A}
			...
			return cop.value !== void 0 ? this._detailValueItem("COP", cop.value.toLocaleString(void 0, { maximumFractionDigits: 2 }), "aus thermischer / elektrischer Leistung berechnet") : A;
              ...
              ${hp.daily_cop ? this._detailItem("Tagesarbeitszahl (JAZ)", hp.daily_cop) : (() => {
			const dailyCop = this._heatPumpDailyCopInfo(hp);
			return dailyCop.value !== void 0 ? this._detailValueItem("Tagesarbeitszahl (JAZ)", dailyCop.value.toLocaleString(void 0, { maximumFractionDigits: 2 }), "aus thermischer / elektrischer Tagesenergie berechnet") : A;
		})()}
            </div>
	...
	_cardStyle() {
		...
		}[this._config.text_size ?? "large"];
		const scale = Math.min(1.3, Math.max(.9, this._config.mobile_scale ?? 1.06));
      ...
      >
        ...
        ${dailyItems.length ? b`
			...
			if ([
				...
				"self-supply-savings"
			].includes(item.key) && item.expandable) {
			...
			if (item.entity) this._openMoreInfo(item.entity);
                ...
                >
                  <span class="daily-item-label">
                    ${item.label}
                    ${item.expandable ? b`<b>${item.key === "pv" ? this._pvDailyExpanded ? "▲" : "▼" : this._dailyBalanceExpanded ? "▲" : "▼"}</b>` : A}
                  </span>
        ...
        <div class="diagram-fit">
          ...
          >
            ...
            ${layout.pvSummary ? this._flowPath(this._connectionPath(layout.pvSummary, layout.center), this._positiveFlow(this._totalPvW()), this._totalPvW(), "pv-summary", [layout.pvSummary.id, layout.center.id]) : layout.pvGroup ? this._flowPath(`M ${layout.pvGroup.x + layout.pvGroup.width / 2} ${layout.pvGroup.y + layout.pvGroup.height} L ${layout.center.x + layout.center.w / 2} ${layout.center.y}`, this._positiveFlow(this._totalPvW()), this._totalPvW(), "pv-group", ["center", ...(this._config.solar ?? []).map((_, index) => `pv-system-${index}`)]) : A}
			...
			return this._flowPath(path, item.direction, item.numericPower ?? this._powerW(item.power), item.node.id, [source.id, item.node.id]);
		})}

            ${layout.pvSummary ? this._node(layout.pvSummary) : A}
