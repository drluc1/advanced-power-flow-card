...
export class AdvancedPowerFlowCard extends LitElement {
  ...
  private _groupedPvLayout(): boolean {
  ...
  private _resolvedPvDetailLevel(): "minimal" | "compact" | "full" {
  ...
  private _compactMpptLines(system: PvSystemConfig): string[] {
    const children = system.children ?? [];
    if (!children.length) return [];

    const entries = children.map((child, index) => {
      const name = this._short(child.name ?? `MPPT ${index + 1}`, 13);
      return `${name} ${this._formatPower(child.power, true)}`;
    });
  ...
  private _layout(): LayoutResult {
        ...
        make: (x, y, nodeW) => ({
          ...
          node: {
            ...
            sub: [
              ...
              this._config.prominent_battery_soc === false ? this._formatSoc(battery.soc) : undefined,
    ...
    const placeGroup = (
      ...
      specs.forEach((spec, index) => {
        ...
        const rowStartX = regionX + (regionW - rowWidth) / 2;
    ...
    } else if (centerSpecs.length) {
      leftRows = placeGroup(centerSpecs, sideMargin, width - sideMargin * 2);
    } else if (houseSpecs.length) {
  ...
  private _node(node: DiagramNode) {
    ...
    const prominentBatterySoc = node.kind === "battery" && node.batterySoc !== undefined && this._config.prominent_battery_soc !== false;
      ...
      >
        ...
        ${node.heatPump
          ...
          : node.batteryIndex !== undefined
            ? svg`<text x=${node.x + node.w - 12} y=${node.y + node.h - 18} text-anchor="end" class="node-action">${this._expandedBattery === node.batteryIndex ? "▲" : "▼"}</text>`
      ...
      >
        <div class="header">
          <div>
            ...
            ${this._config.show_live_kpis === false
              ...
              ${batteries.map((battery, index) => {
                ...
                return html`
                  <button
                    ...
                    @click=${() => {
                      const next = this._expandedBattery === index ? undefined : index;
                      this._expandedBattery = next;
                      if (next !== undefined) {
                        this._heatExpanded = false;
                        this._pvDailyExpanded = false;
                  ...
                  >
                    ...
                    <div class="live-kpi-copy">
                      ...
                      <strong>${soc === undefined ? "—" : `${Math.round(soc)} %`}</strong>
